...
(function (YAGL) {
     ...
     var Graph = (function () {
        ...
        Graph.prototype.BFSearch = function (startVid, stopVid) {
            ...
            queue.push(startVid); //add it to queue
            ...
            var curVid;
            while (queue.length !== 0) {
                curVid = queue.shift(); //remove vertex from queue
                if (curVid === stopVid){
                    console.log("Returned curvid: " + paths[curVid]);
                    return paths[curVid];      //returns when its found
                }
                ...
                for (eid in list) {
                    ...
                    if (v.getVisited() === false) {
                        ...
                        for(var i = 0; i < queue.length; i++) {
                            if(queue[i] == queue[v.getVid()]){
                              check = true;
                            //  paths[v.getVid()] = paths[startVid];
                            }
                          }
                          if(check) {
                            var value = paths[v.getParent()] + 1;
                            paths[v.getVid()] = value;
                            console.log(" if paths[v.getvid] "+ value);
                          }
                          else {
                              var value = paths[v.getParent()];
                              paths[v.getVid()] = value;
