...
(function (YAGL) {
     ...
     var Graph = (function () {
        ...
        /*
         *  getAllEdgeCounts() returns hashmap of a each vid's number of edges in graph
         */
        Graph.prototype.getAllEdgeCounts = function () {
        var set = {};
        var i;
         ...
         * ALERT -------- changed return value from current vid when found to
         *                TOTAL NUMBER OF SHROTEST PATHS -------------ALERT
         */
        ...
        /* adaption from C++ code found
         * http://www.geeksforgeeks.org/find-paths-given-source-destination/
         *
         */
        Graph.prototype.setAllPaths = function (s, d) {
            var num_nodes = Object.keys(this.vertices).length;
            ...
            var paths = [];
            var path_index = 0; //initialize path[] as empty

            //call the recursive helper fucntion to print all paths
            this.allPathsUtil(s, d, paths, path_index);
        };

        Graph.prototype.printAllPaths = function () {
          /*console.log(" " + allPaths.length);
          for(var i = 0; i < allPaths.length; i++){
              console.log("Path " + i);
              console.log(" paths " + allPaths[i]);
          }
          */
          return allPaths;
        }

        Graph.prototype.allPathsUtil = function (s, d, paths, path_index) {
            //mark the current node and store it in path
            this.vertices[s].setVisited(true);
            paths[path_index] = s;
            path_index++;
            ...
            if(s == d) {
              ...
              allPaths[allPaths.length] = check;
            }
            else {  //if currrent v is not in destination
              var list = this.adjacencyList[s];
              var e, adjVid, v;
              //recur for all the vertices adajacent to the current vertex
              for (eid in list) {
                  e = this.edges[eid];
                  adjVid = e.getAdjacentVertex(s);
                  v = this.vertices[adjVid];
                  if (!v.getVisited()) {
                     this.allPathsUtil(adjVid, d, paths, path_index);
                  }
              }
          }
          //remove current vertex from path[] and mark it as unvisited
          path_index--;
          this.vertices[s].setVisited(false);
        };
